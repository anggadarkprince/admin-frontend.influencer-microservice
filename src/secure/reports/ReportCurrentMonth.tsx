...
class ReportCurrentMonth extends Component<{user: User}> {
    state = {
        isLoading: true,
        fromPageOrder: 1,
        lastPage: 1,
    ...
    componentDidMount = async () => {
        document.title = 'Report Current Months';

        this.setState({
            isLoading: true
        });

        const response = await axios.get(`reports/current-month?page=${this.page}`);
    ...
    render() {
        return (
            <Wrapper>
                <SectionTitle title={"Report Current Month"}/>
...
export default connectToRedux(ReportCurrentMonth);