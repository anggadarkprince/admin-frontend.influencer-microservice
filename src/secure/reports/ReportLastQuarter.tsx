...
class ReportLastQuarter extends Component<{user: User}> {
    state = {
        isLoading: true,
        fromPageOrder: 1,
        lastPage: 1,
    ...
    componentDidMount = async () => {
        document.title = 'Report Last Quarters';

        this.setState({
            isLoading: true
        });

        const response = await axios.get(`reports/last-quarter?page=${this.page}`);
    ...
    render() {
        return (
            <Wrapper>
                <SectionTitle title={"Report Last Quarters"}/>
...
export default connectToRedux(ReportLastQuarter);